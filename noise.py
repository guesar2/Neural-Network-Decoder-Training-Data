...
class NoiseModel:
    ...
    def noisy_op(self, op: stim.CircuitInstruction, p: float, ancilla: int) -> Tuple[stim.Circuit, stim.Circuit, stim.Circuit]:
        ...
        if p > 0:
            if op.name in ANY_CLIFFORD_1_OPS:
            ...
            elif op.name in RESET_OPS or op.name in MEASURE_OPS or op.name == "MR":
                ...
                if op.name == "MR":
                    pre.append("X_ERROR", targets, (5/2) * p)
                    post.append("X_ERROR", targets, p)
            elif op.name == "MPP":
    ...
    def noisy_circuit(self,
        ...
        for op in circuit:
            if isinstance(op, stim.CircuitRepeatBlock):
                ...
                if op.name in MEASURE_OPS or op.name in RESET_OPS or op.name == "MR":
